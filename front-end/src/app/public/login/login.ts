    ...
    <div class="min-h-screen flex items-center justify-center bg-[var(--bg-page)] p-4 relative">
      ...
      <div class="w-full max-w-sm">
        <div class="text-center mb-8">
          ...
          <p class="text-text-secondary text-sm mt-1">{{ translation.translate('تسجيل الدخول إلى حسابك', 'Sign in to your account') }}</p>
        </div>
        ...
        <div class="bg-[var(--bg-card)] rounded-2xl shadow-[var(--shadow-card)] p-8">
            ...
            <div class="space-y-5">
              ...
              >
                @if (f().submitting()) {
                  ...
                  {{ translation.translate('جاري...', 'Signing in...') }}
                } @else {
                  {{ translation.translate('تسجيل دخول', 'Sign In') }}
                }
              </button>
            </div>
          </form>
          ...
          <p class="text-center text-sm text-text-secondary mt-6">
            {{ translation.translate('ليس لديك حساب؟', "Don't have an account?") }}
            <a routerLink="/register" class="text-brand-primary font-medium hover:underline">{{ translation.translate('إنشاء حساب', 'Sign up') }}</a>
          </p>
        </div>
      </div>
    </div>
...
export class Login {
  ...
  readonly f = form(this.model, (p) => {
    ...
    submission: {
      action: async () => {
        this.notification.success(this.translation.translate('تم تسجيل الدخول بنجاح', 'Signed in successfully'));
